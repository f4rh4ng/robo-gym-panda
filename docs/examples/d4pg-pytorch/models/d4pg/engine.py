...
class Engine(object):
    ...
    def train_from_checkpoint(self):
    	# ADDED BY FARHANG. For training from checkpoint 
        config = self.config
        checkpoint_path = config['load_path']
        ...
        # Learner (neural net training process)
        target_policy_net = torch.load(checkpoint_path)
        policy_net = copy.deepcopy(target_policy_net)
        policy_net_cpu = torch.load(checkpoint_path, map_location=torch.device('cpu'))
        policy_net_cpu.device ='cpu'
        target_policy_net.share_memory()
    ...
    def test(self):
    	# ADDED BY FARHANG. For evaluation we only use the exploitation agent. 
        config = self.config
        ...
        update_step = mp.Value('i', 0)
        training_on = mp.Value('i', 1)
        learner_w_queue = torch_mp.Queue(maxsize=0)
        replay_priorities_queue = mp.Queue(maxsize=config['replay_queue_size'])
        # Create directory for experiment
        experiment_dir = f"{config['results_path']}/{config['env']}-{config['model']}-{datetime.now():%Y-%m-%d_%H:%M:%S}"
        if not os.path.exists(experiment_dir):
        ...
        agent.run(training_on=training_on, replay_queue=replay_priorities_queue, learner_w_queue=learner_w_queue, update_step=update_step)
