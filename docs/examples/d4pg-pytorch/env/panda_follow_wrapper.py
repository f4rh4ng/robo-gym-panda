...
class PandaFollowWrapper:
    def __init__(self, agent_type):
        self.agent_type = agent_type
        if agent_type == "exploration":
            target_machine_ip = '127.0.0.1'
            self.env = gym.make('FollowPandaSim-v0', ip=target_machine_ip, gui=False)
            self.env = ExceptionHandling(self.env)
        else:
            target_machine_ip = '127.0.0.1'
            self.env = gym.make('FollowPandaSim-v0', ip=target_machine_ip, gui=True)
            self.env = ExceptionHandling(self.env)
